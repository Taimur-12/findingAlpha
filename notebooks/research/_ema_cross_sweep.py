"""
EMA20 cross strategy.

LONG: close crosses above EMA20 (prev_close < ema20 <= close) in trend_up, ADX >= 20
SHORT: close crosses below EMA20 (prev_close > ema20 >= close) in trend_down, ADX >= 20

Also tests: prev_close is available via FeatureSnapshot.prev_close? No — need different approach.
We need to check if the *current bar* crossed ema20. Strategy adapter gets `row` AND `snapshot`.
The event_runner calls: fn(snapshot, regime, row, now).
We need the previous row to check the cross — event_runner doesn't pass that.

Alternative approach: use bar.open vs ema20 as a proxy for "where price was at start of bar".
LONG: bar.open < ema20 AND close > ema20 (intra-bar cross above)
SHORT: bar.open > ema20 AND close < ema20 (intra-bar cross below)

Test this plus variations.
"""
...
def _make_cross_fn(
    stop_atr: float, target_atr: float, min_adx: float,
    use_vol_z: bool, min_vol_z: float,
    stop_anchor: str,  # "entry" or "ema50"
    horizon: int = 720,
):
    strat_id = f"emacross_{stop_atr:.2f}_{target_atr:.1f}_{stop_anchor}_adx{min_adx:.0f}"
    if use_vol_z:
        strat_id += f"_vz{min_vol_z:.1f}"
    req = list(_REQUIRED)
    if use_vol_z:
...
def main():
    ...
    # First: count raw intra-bar EMA20 crosses (no regime filter) to understand frequency
    feat_df = None
    try:
        from finding_alpha.features.snapshot import build_feature_df
        feat_df = build_feature_df(candles, funding, oi)
        long_crosses = (feat_df["open"].astype(float) < feat_df["ema_20"]) & (feat_df["close"].astype(float) > feat_df["ema_20"])
        short_crosses = (feat_df["open"].astype(float) > feat_df["ema_20"]) & (feat_df["close"].astype(float) < feat_df["ema_20"])
        print(f"Raw EMA20 cross-above count: {long_crosses.sum()}")
        print(f"Raw EMA20 cross-below count: {short_crosses.sum()}")
        print(f"Total bars: {len(feat_df)}")
    except Exception as e:
        print(f"Feature df check failed: {e}")

    variants = [
        dict(stop_atr=0.5,  target_atr=2.5, min_adx=20, use_vol_z=False, min_vol_z=0, stop_anchor="ema50"),
        dict(stop_atr=0.5,  target_atr=2.5, min_adx=15, use_vol_z=False, min_vol_z=0, stop_anchor="ema50"),
        dict(stop_atr=0.5,  target_atr=4.5, min_adx=15, use_vol_z=False, min_vol_z=0, stop_anchor="ema50"),
        dict(stop_atr=0.5,  target_atr=2.5, min_adx=20, use_vol_z=True,  min_vol_z=1.0, stop_anchor="ema50"),
        dict(stop_atr=0.5,  target_atr=2.5, min_adx=20, use_vol_z=True,  min_vol_z=0.5, stop_anchor="ema50"),
        dict(stop_atr=0.5,  target_atr=4.5, min_adx=20, use_vol_z=True,  min_vol_z=1.0, stop_anchor="ema50"),
        dict(stop_atr=0.75, target_atr=4.5, min_adx=20, use_vol_z=True,  min_vol_z=1.0, stop_anchor="ema50"),
        dict(stop_atr=0.5,  target_atr=2.5, min_adx=20, use_vol_z=False, min_vol_z=0, stop_anchor="entry"),
        dict(stop_atr=0.75, target_atr=4.5, min_adx=15, use_vol_z=False, min_vol_z=0, stop_anchor="entry"),
    ]

    print(f"\n{'anchor':>8} {'stop':>5} {'tgt':>5} {'adx':>4} {'volz':>5} | "
          f"{'sigs':>6} {'trd':>6} {'win%':>6} {'exp_r':>7} {'PF':>6} {'wf':>8} | GATE")
    print("-" * 100)

    for v in variants:
        fn, strat_id = _make_cross_fn(**v)
