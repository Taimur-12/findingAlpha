"""
Phase 8 paper trading runner — short_composite_v1.
...
Usage:
    ...
    python notebooks/phase8_short_composite_runner.py

    # Custom poll interval:
    python notebooks/phase8_short_composite_runner.py --poll 120

    # Print current paper status without processing:
    python notebooks/phase8_short_composite_runner.py --status

All paper data is written to the paper/composite/ directory:
    paper/composite/state.json      — current account state
    paper/composite/trades.jsonl    — append-only closed trade log
    paper/composite/matrix.jsonl    — full Matrix event audit log

Frozen Phase 8 parameters (do not change during observation period):
    strategy:  short_composite_v1
    symbol:    BTCUSDT
    timeframe: 1h
    risk/trade: 0.25%
    max_hold:  12h
    Signal 1 (breakdown): close < prev_day_low, vol_z >= 1.0
      stop: entry + 0.75 ATR  target: entry - 4.5 ATR
    Signal 2 (EMA rejection): bar.open > EMA20 >= close, ADX >= 20, trend_down
      stop: EMA50 + 0.5 ATR   target: close - 4.5 ATR
"""
...
PAPER_DIR = _root / "paper" / "composite"
...
def print_status() -> None:
    ...
    print("=" * 60)
    print("  PHASE 8 PAPER STATUS — short_composite_v1")
    print("=" * 60)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase 8 paper runner — short_composite_v1")
